...
03. 选择双盘, 选择双盘, 选择双盘
04. 默认安装, 重启。再安装一遍Acer保护卡, 调整参数。
05. 安装完成时的状态就是根进度。
06. 新机的话, 先传一遍底层, 再传系统。
07. 后续新增软件, 要先切换到个人模式, 装好软件等相关操作后, 创建进度, 选择进度, 保存进度。(每次开机重启时候的进度, 可以选择)(进度可以删除和创建)
08. 然后再切换到机房模式。
09. 选择增量传输。
10. 系统传输一般在20-30分钟之间, 底层和增量传输在1分钟内。
11. 另注: 在网络克隆准备工作中,出现的IP冲突, 可以全部按ID和IP顺序进行重新排列分配, 调整好后, 再传一遍底层参数。
12. 遇见169.前缀的IP, 重启一下交换机, 针对某项目, 直接关后面的插板。

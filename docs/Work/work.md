...
* **[统信UOS生态社区](https://www.chinauos.com/)**
  - [资源中心 | 统信UOS生态社区](https://www.chinauos.com/resource/download-professional)
...
---
**[装机必备软件-六一库·待查]{s}**
* **[六一库](https://www.61ku.com/software)**
...
### 0009. Acer保护卡安装步骤:
* **[Acer保护卡安装步骤]{s}**
  - Windows防火墙和Window更新关闭, 账户控制调到最低。
  - 安装Acer保护卡 V2.7.2 版本。
  - 选择双盘, 选择双盘, 选择双盘
  - 默认安装, 重启。再安装一遍Acer保护卡, 调整参数。
  - 安装完成时的状态就是根进度。
  - 新机的话, 先传一遍底层, 再传系统。
  - 后续新增软件, 要先切换到个人模式, 装好软件等相关操作后, 创建进度, 选择进度, 保存进度。(每次开机重启时候的进度, 可以选择)(进度可以删除和创建)
  - 然后再切换到机房模式。
  - 选择增量传输。
  - 系统传输一般在20-30分钟之间, 底层和增量传输在1分钟内。
  - 另注: 在网络克隆准备工作中,出现的IP冲突, 可以全部按ID和IP顺序进行重新排列分配, 调整好后, 再传一遍底层参数。
